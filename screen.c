 ...
 * Copyright (C) 1999-2007 Ciaran Anscomb <xwm@6809.org.uk>
	...
	if (hv & MAXIMISE_FIT) {
		if (c->oldw) {
			...
			if (c->oldh) {
				...
				c->x = 0;
				c->y = 0;
				c->width = DisplayWidth(dpy, c->screen->screen)-CONKY_WIDTH;
				c->height = DisplayHeight(dpy, c->screen->screen);
			}
		}
	}
...
static KeySym keys_to_grab[] = {
	...
	KEY_LOWER, KEY_ALTLOWER, KEY_INFO, KEY_MAXVERT, KEY_MAX, KEY_MAXFIT
};
